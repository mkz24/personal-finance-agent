...
h1,h2,h3,h4,h5,h6,p,li,label,span,.stMarkdown { color: #e2e8f0; }
</style>
...
class AnalyzerAgent:
    def analyze(self, df):
        ...
        return "\n".join(summary_lines), data, income, expenses_abs, net
...
class CriticAgent:
    def critique(self, plan_text, df):
        ...
        if net < 0:           risks.append(f"Negative net (${net:,.2f}) — you are spending more than you earn!")
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 1rem 0 1.5rem 0;'>
        <div style='font-size:1.5rem; font-weight:800; color:#a78bfa;'>Finance Agent</div>
    ...
    steps = [
        ...
        ("3.", "Click Plan to get a savings strategy",     st.session_state.plan is not None),
        ("4.", "Click Critique to spot financial risks",   st.session_state.critique is not None),
        ("5.", "Ask the AI Advisor anything",              False),
    ]
    for icon, text, done in steps:
        cls = "done" if done else ""
        st.markdown(
            f'<div class="sidebar-step {cls}"><b>{icon}</b> {text}</div>',
            unsafe_allow_html=True
        )
...
<div class="hero-banner">
    ...
    <p>Upload your bank statement &middot; Get AI-powered insights &middot; Take control of your money</p>
</div>
""", unsafe_allow_html=True)
...
if st.session_state.income_val is not None:
    ...
    net_label = "Up" if net >= 0 else "Down"
    st.markdown(f"""
    <div class="metric-row">
        ...
        <div class="metric-card {net_class}">
            <div class="label">Net Balance ({net_label})</div>
            <div class="value">${net:,.0f}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)

if any([st.session_state.summary, st.session_state.plan,
        st.session_state.critique, st.session_state.analyzed_df is not None]):
